...
export function Register() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center overflow-auto transition-colors duration-500" style={{ background: bg }}>
      ...
      <div className="w-full max-w-md px-6 py-8">
        ...
        <div className="backdrop-blur-sm rounded-xl p-8" style={{ background: cardBg, border: `1px solid ${cardBorder}`, boxShadow: isDark ? 'none' : '0 8px 32px rgba(0,0,0,0.08)' }}>
          <h2 className="mb-6 text-center" style={{ color: textColor }}>Create New Account</h2>

          <form onSubmit={handleRegister} className="space-y-4">
            <div>
              <label className="text-sm block mb-2" style={{ color: mutedColor }}>Full Name</label>
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5" style={{ color: mutedColor }} />
            ...
            <div>
              <label className="text-sm block mb-2" style={{ color: mutedColor }}>Confirm Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5" style={{ color: mutedColor }} />
          ...
          <div className="mt-6 text-center">
            <span className="text-sm" style={{ color: mutedColor }}>Already have an account? </span>
            <button onClick={() => router.push('/login')} className="text-purple-500 hover:text-purple-400 text-sm">
              Login here
            </button>
          </div>
        </div>
